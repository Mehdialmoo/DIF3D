"""
Module for 3D rendering and image processing.

This module contains various classes and functions
for 3D rendering and image processing,
including:
- BaseModule:
    A base module class for other modules to inherit from.
- ImagePreprocessor:
    A class for preprocessing images.
- rays_intersect_bbox:
    A function for checking if rays intersect with a bounding box.
- chunk_batch:
    A function for chunking a batch of data and
    applying a function to each chunk.
- scale_tensor:
    A function for scaling a tensor.
- get_activation:
    A function for getting an activation function by name.
- get_ray_directions:
    A function for getting ray directions for all pixels in camera coordinate.
- get_rays: A function for getting rays in the world coordinate.
- get_spherical_cameras:
    A function for getting spherical cameras.
- remove_background:
    A function for removing the background of an image.
- resize_foreground:
    A function for resizing the foreground of an image.
- save_video:
    A function for saving a list of frames as a video.
- to_gradio_3d_orientation:
    A function for converting a mesh to a format suitable
    for Gradio 3D visualization.
...
# Function to parse structured fields
def parse_structured(
        fields: Any,
        cfg: Optional[Union[dict, DictConfig]] = None) -> Any:

    scfg = OmegaConf.merge(OmegaConf.structured(fields), cfg)
    return scfg


# Function to find a class by string
...
# Function to get intrinsic from field of view
...
class BaseModule(nn.Module):
    ...
    # add this to every subclass of BaseModule
...
class ImagePreprocessor:
    def convert_and_resize(
        ...
        # a torch tensor if it is a PIL Image or numpy array
        ...
        # Check if the input tensor is batched
        batched = image.ndim == 4

        # Add a batch dimension if the input tensor is not batched
        if not batched:
            image = image[None, ...]
...
# Function to check if rays intersect with a bounding box
def rays_intersect_bbox(
    ...
    # tighten the radius to make sure
    # the intersection point lies in the bounding box
...
def chunk_batch(func: Callable, chunk_size: int, *args, **kwargs) -> Any:
    """
    This function is used to split a batch into smaller chunks
    and apply a function to each chunk. If chunk_size is less than
    or equal to 0, the function is applied to the entire batch.
    If no tensor is found in args or kwargs, an error is raised.
    The function returns the output of the function applied to each chunk,
    ...
    for i in range(0, max(1, B), chunk_size):
        ...
        if isinstance(out_chunk, torch.Tensor):
            ...
            print(
                f"Return value of func must be in type \
                    [torch.Tensor, list, tuple, dict], get {type(out_chunk)}."
            )
            exit(1)
    ...
    for k, v in out.items():
        if all([vv is None for vv in v]):
            ...
            val = [type(vv) for vv in v if not isinstance(vv, torch.Tensor)]
            raise TypeError(
                f"Unsupported types in return value of func: {val}"
            )
...
def scale_tensor(
    ...
    # If input scale is not provided, set it to (0, 1)
    if inp_scale is None:
        inp_scale = (0, 1)
    # If target scale is not provided, set it to (0, 1)
    if tgt_scale is None:
        tgt_scale = (0, 1)
    # If target scale is a tensor,
    # check if its shape matches the last dimension of dat
    if isinstance(tgt_scale, torch.FloatTensor):
        assert dat.shape[-1] == tgt_scale.shape[-1]
    # Scale the data to the target range
...
def get_ray_directions(
    ...
    Inputs:
        H, W, focal, principal, use_pixel_centers: image height, width,
        focal length, principal point and whether use pixel centers
    Outputs:
...
def get_rays(
    ...
    if directions.ndim == 2:  # (N_rays, 3)
        ...
        rays_d = (
            directions[:, :, :, None, :] * c2w[:, None, None, :3, :3]).sum(
            -1
        )  # (B, H, W, 3)
        rays_o = c2w[:, None, None, :3, 3].expand(rays_d.shape)
...
def save_video(
    ...
    # Saves a list of PIL Image objects as a video.
    # Convert PIL Image objects to numpy arrays
    frames = [np.array(frame) for frame in frames]

    # Create a writer object with the specified output path and fps
